...
function initial(){
    tab.setOption({
        title : {
            text: '2013-2017年'+currentcity+'固定资产投资',
            x:'left'
        },
        tooltip : {
            trigger: 'axis',
...
function flushData(){
    ...
    for(var year in years)
        if(valueOrPer)
            data = data.concat(consout[city_index[currentcity]][years[year]]);
        else
            data = data.concat(((consout[city_index[currentcity]][years[year]]/outsum[city_index[currentcity]][years[year]]*100)-0.01).toFixed(2));

    var serie = {"name": "房地产开发投资",
                    "type": 'line',
                    "stack": 'cons',
                    "label": {
                    "normal": {
    ...
    for(var year in years)
        if(valueOrPer)
            sumdata = sumdata.concat(outsum[city_index[currentcity]][years[year]]);
        else
    ...
    option.title[0].text = '2013-2017年'+currentcity+'固定资产投资';
